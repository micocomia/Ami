...
2c. **Abstract Priority — Session Coverage First**: The `abstract` must primarily describe the session's topic, scope, and learner-facing outcome. Write the abstract in this order:
   - First: what the session covers.
...
2d. **Abstract-Flag Consistency (MANDATORY)**: The `abstract` field MUST accurately reflect the session's actual flags. Violations will cause the path to be rejected. Enforce the following rules without exception:
   ...
   - **`session_sequence_hint`**: The abstract narrative MUST follow the hint's ordering:
     ...
     * `"application-first"`: Abstract must describe a concrete example, task, or application *before* theory is explained.
     * `null` (no hint): Balanced order; do not impose a strong sequencing direction.
   - **`thinking_time_buffer_minutes`**: If > 0, the abstract MUST mention a brief reflection pause or reflection period. If 0, do NOT mention one.
   - **`input_mode_hint`**: The abstract's framing must match the mode:
     * `"visual"`: Reference diagrams, charts, or visual overviews in the abstract with brief wording; do not let visual-format language dominate the abstract.
     * `"verbal"`: Frame the session as rich written explanations and narrative descriptions (e.g., stories or analogies). For strong verbal learners (> +0.7), note that a podcast-style audio experience will accompany this session. Do NOT reference diagrams as the primary medium, and keep verbal-format language secondary to session substance.
     * `"mixed"`: May reference both textual and visual elements; do NOT mention audio or podcast.
   - **Substance over Delivery**: The abstract MUST make the session substance legible on its own. A reader should be able to tell what the learner will study and achieve even if all adaptation phrases are removed.
